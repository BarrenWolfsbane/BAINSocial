...
public class GnuPGSignature extends GnuPGPeer {
     ...
     */
    protected GnuPGSignature(long ptr) { setInternalRepresentation(ptr); } // note that this is a pointer to an address in the gnupg-for-java shared lib
    /**
     * Is signature key revoked?
     * 
     * @return true, if revoked
     */
    public boolean isRevoked() { return gpgmeGetRevoked(getInternalRepresentation()); }
    /**
     * Is signature key expired?
     * 
     * @return true, if expired
     */
    public boolean isExpired() { return gpgmeGetExpired(getInternalRepresentation()); }
    /**
     * Is signature key invalid?
     * 
     * @return true, if invalid
     */
    public boolean isInvalid() { return gpgmeGetInvalid(getInternalRepresentation()); }
    /**
     * Is signature key exportable?
     * 
     * @return true, if exportable
     */
    public boolean isExportable() { return gpgmeGetExportable(getInternalRepresentation()); }
    /**
     ...
     */
    public boolean hasDetails() { return !getName().equals(""); }
    /**
    ...
    public String toString() {
        ...
        else buf.append(" <No details available.>");
        return buf.toString();
    }
    /**
    ...
    protected GnuPGSignature getNextSignature() {
        ...
        if (next != 0) result = new GnuPGSignature(next);
        return result;
    }
